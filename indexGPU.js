...
const benchedd = require('./GPU_bench.json');
const db = require('./GPU.json');
const fs = require('fs');
...
db.forEach(item => {
    benched.forEach(bench => {
        if (bench[0].toLowerCase().indexOf(item.title.toLowerCase()) !== -1 && used.indexOf(bench[0].toLowerCase()) === -1) {
            if (
                (bench[0].toLowerCase().indexOf('x8') !== -1 || bench[0].toLowerCase().indexOf('x 2') !== -1 || bench[0].toLowerCase().indexOf('x4') !== -1) &&
                used.indexOf(bench[0].toLowerCase()) === -1
            ) {
                console.log(bench[2]);
            }
            let obj = Object.assign({}, item);
            obj.HashRateBench = [bench[2]];
            dist.push(obj);
            ...
            dist.forEach((el, i) => {
                if (bench[0].toLowerCase().indexOf(el.title.toLowerCase()) !== -1) {
                    dist[i].HashRateBench.push(bench[2]);
